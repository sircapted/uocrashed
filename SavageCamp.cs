...
{	
	public class SavageCamp : BaseCamp
	{
		public virtual Mobile Savages{ get{ return new Savage(); } }
		public virtual Mobile Executioners{ get{ return new Executioner(); } }
		
		private Mobile m_Prisoner;
		
		[Constructable]
		public SavageCamp() : base( 0x10EE ) // dummy garbage at center
		{
		}
		...
		public override void AddComponents()
		{
			...
			for ( int i = 0; i < 4; i ++ )
			{				
				AddMobile( Savages, 6, Utility.RandomMinMax( -7, 7 ), Utility.RandomMinMax( -7, 7 ), 0 );
			}
		...
		private void AddCampChests()
		{
			...
			crates.RequiredSkill = 96;
			crates.LockLevel = 96;
			crates.MaxLockLevel = 116;
			crates.Locked = true;

			crates.DropItem(new Gold(Utility.RandomMinMax(1000, 4000)));
			crates.DropItem(new Arrow(100));
			crates.DropItem(new Bolt(100));
			...
			if (Utility.RandomDouble() < 0.8)
			{
				switch (Utility.Random(4))
				{
					case 0: crates.DropItem(new GreaterCurePotion()); break;
					case 1: crates.DropItem(new LesserExplosionPotion()); break;
					case 2: crates.DropItem(new GreaterHealPotion()); break;
		...
		public SavageCamp( Serial serial ) : base( serial )
		{
		}
